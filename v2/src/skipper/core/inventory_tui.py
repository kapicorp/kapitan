...
class TargetList(DataTable):
    ...
    def __init__(self, targets: list[TargetInfo], **kwargs):
    ...
    def get_selected_target(self) -> TargetInfo | None:
    ...
    def update_targets(self, targets: list[TargetInfo]) -> None:
...
class JSONPathSearch(Container):
    ...
    def set_all_targets(self, targets: list[TargetInfo]) -> None:
        """Set all targets for JSONPath searching."""
        self.all_targets = targets
        if targets:
            # Convert TargetInfo objects to dict for JSONPath completer
            targets_dict = [target.model_dump() for target in targets]
            self.completer = JSONPathCompleter({"targets": targets_dict})
    ...
    def set_selected_target(self, target_name: str) -> None:
        ...
        for target in self.all_targets:
            if target.name == target_name:
                self.current_data = target.parameters
                break
        ...
        try:
            ...
            if not query.strip():
                ...
                if self.current_data:
                    ...
                    targets_dict = [target.model_dump() for target in self.all_targets]
                    json_viewer.update_content({"all_targets": targets_dict})
                return
            ...
            if query.startswith("$..[name='") and ("']" in query):
                ...
                if not remaining_path:
                    ...
                    json_viewer.update_content(target_dict)
                    self.current_data = target_dict
                    return
            ...
            # Convert TargetInfo objects to dictionaries for JSONPath processing
            targets_dict = [target.model_dump() for target in self.all_targets]
            jsonpath_expr = jsonpath_parse(query)
            matches = jsonpath_expr.find(targets_dict)

            if matches:
                result_data = [match.value for match in matches]
                # Single match - show the value directly, otherwise show as array
                filtered_data = result_data[0] if len(result_data) == 1 else result_data

                json_viewer.update_content(filtered_data)
                self.current_data = filtered_data  # Cache successful result
            else:
                # No matches found - silently keep current view (no notification spam)
                pass

        except Exception:
            # Silently handle invalid queries while typing (no notification spam)
            pass
...
class InventoryTUIApp(App):
    ...
    def _update_target_details(self, target: TargetInfo) -> None:
        ...
        try:
            ...
            search_panel.set_selected_target(target.name)
    ...
    def _update_left_panel_width(self) -> None:
        ...
        max_length = max(len(target.name) for target in self.targets)
...
def show_interactive_inventory(console, inventory_path: str = "inventory") -> None:
    ...
    try:
        ...
        console.print("[yellow]Please check your terminal supports advanced TUI features[/yellow]")
        raise
