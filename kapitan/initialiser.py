...
class initialise(object):
    ...
    def __init__(self,directory,target,compile_input,classes):
        ...
        self.class_template = os.path.join(
                    self.templates_directory,'inventory','classes','my_component.yml'
                )
        self.target_template = os.path.join(
                    self.templates_directory,'inventory','targets','my_target.yml'
                )


    def __call__(self):
        copytree(self.templates_directory, self.directory, ignore=ignore_patterns('my*.yml'))
    ...
    def initialise_class(self):
        ...
        for value in compile_target['parameters']['kapitan']['compile']:
            if value['input_type'] not in self.compile_input:
                compile_target['parameters']['kapitan']['compile'].remove(value)
