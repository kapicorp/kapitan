...
class GrammarVisitor(OmegaConfGrammarParserVisitor):
    ...
    def visitConfigKey(self, ctx: OmegaConfGrammarParser.ConfigKeyContext) -> str:
        ...
        if isinstance(child, OmegaConfGrammarParser.InterpolationContext):
            ...
            assert isinstance(child, TerminalNode) and isinstance(child.symbol.text, str)
    ...
    def visitDictContainer(self, ctx: OmegaConfGrammarParser.DictContainerContext) -> Dict[Any, Any]:
        # BRACE_OPEN (dictKeyValuePair (COMMA dictKeyValuePair)*)? BRACE_CLOSE
        assert ctx.getChildCount() >= 2
        return dict(self.visitDictKeyValuePair(ctx.getChild(i)) for i in range(1, ctx.getChildCount() - 1, 2))
    ...
    def visitInterpolation(self, ctx: OmegaConfGrammarParser.InterpolationContext) -> Any:
    ...
    def visitInterpolationResolver(self, ctx: OmegaConfGrammarParser.InterpolationResolverContext) -> Any:
    ...
    def visitDictKeyValuePair(self, ctx: OmegaConfGrammarParser.DictKeyValuePairContext) -> Tuple[Any, Any]:
        from ._utils import _get_value

        assert ctx.getChildCount() == 3  # dictKey COLON element
        key = self.visit(ctx.getChild(0))
        colon = ctx.getChild(1)
        assert isinstance(colon, TerminalNode) and colon.symbol.type == OmegaConfGrammarLexer.COLON
        value = _get_value(self.visitElement(ctx.getChild(2)))
        return key, value

    def visitListContainer(self, ctx: OmegaConfGrammarParser.ListContainerContext) -> List[Any]:
    ...
    def visitSequence(self, ctx: OmegaConfGrammarParser.SequenceContext) -> Generator[Any, None, None]:
        from ._utils import _get_value
        ...
        for child in ctx.getChildren():
            if isinstance(child, OmegaConfGrammarParser.ElementContext):
                ...
                assert isinstance(child, TerminalNode) and child.symbol.type == OmegaConfGrammarLexer.COMMA
    ...
    def visitSingleElement(self, ctx: OmegaConfGrammarParser.SingleElementContext) -> Any:
        # element EOF
        assert ctx.getChildCount() == 2
        return self.visit(ctx.getChild(0))
            ...
            if isinstance(node, TerminalNode):
                ...
                if s.type == OmegaConfGrammarLexer.ESC_INTER:
                    ...
                    or (
                        ...
                        and isinstance(next_node, OmegaConfGrammarParser.InterpolationContext)
                    )
                    or (
                        ...
                        and (
                            next_node is None
                            or isinstance(next_node, OmegaConfGrammarParser.InterpolationContext)
                        )
                    )
                ):
