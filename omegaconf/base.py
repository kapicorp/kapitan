    ...
    def _dereference_node_impl(
        ...
        if parent is None:
            if throw_on_resolution_failure:
                raise InterpolationResolutionError("Cannot resolve interpolation for a node without a parent")
            return None
...
class Container(Box):
    ...
    def _resolve_interpolation_from_parse_tree(
        ...
        try:
            resolved = self.resolve_parse_tree(parse_tree=parse_tree, node=value, key=key, memo=memo)
    ...
    def _validate_not_dereferencing_to_parent(self, node: Node, target: Node) -> None:
        ...
        while parent is not None:
            if parent is target:
                raise InterpolationResolutionError("Interpolation to parent node detected")
            parent = parent._get_parent()

    def _resolve_node_interpolation(self, inter_key: str, memo: Optional[Set[int]]) -> "Node":
    ...
    def _evaluate_custom_resolver(
        ...
        if resolver is not None:
            ...
            raise UnsupportedInterpolationType(f"Unsupported interpolation type {inter_type}")
    ...
    def resolve_parse_tree(
        ...
        def node_interpolation_callback(inter_key: str, memo: Optional[Set[int]]) -> Optional["Node"]:
