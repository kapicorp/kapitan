...
class ValueNode(Node):
    ...
    def _set_value(self, value: Any, flags: Optional[Dict[str, bool]] = None) -> None:
        ...
        if isinstance(value, str) and get_value_kind(value, strict_interpolation_validation=True) in (
    ...
    def validate_and_convert(self, value: Any) -> Any:
        ...
        if value is None:
            ...
            raise ValidationError(f"Incompatible value '{value}' for field of type '{ref_type_str}'")
...
class AnyNode(ValueNode):
    def __init__(
        ...
        super().__init__(
            parent=parent,
            value=value,
            metadata=Metadata(ref_type=Any, object_type=None, key=key, optional=True, flags=flags),
        )
    ...
    def _validate_and_convert_impl(self, value: Any) -> Any:
        ...
        if self._get_flag("allow_objects") is not True and not is_primitive_type_annotation(value):
            t = get_type_of(value)
            raise UnsupportedValueType(f"Value '{t.__name__}' is not a supported primitive type")
        return value
...
class StringNode(ValueNode):
    ...
    def _validate_and_convert_impl(self, value: Any) -> str:
        ...
        if OmegaConf.is_config(value) or is_primitive_container(value) or isinstance(value, bytes):
...
class PathNode(ValueNode):
    ...
    def _strict_validate_type(self, value: Any) -> None:
        if not isinstance(value, Path):
            raise ValidationError("Value '$VALUE' of type '$VALUE_TYPE' is not an instance of 'pathlib.Path'")

    def _validate_and_convert_impl(self, value: Any) -> Path:
        if not isinstance(value, (str, Path)):
            raise ValidationError("Value '$VALUE' of type '$VALUE_TYPE' could not be converted to Path")
...
class IntegerNode(ValueNode):
    ...
    def _validate_and_convert_impl(self, value: Any) -> int:
        try:
            ...
            raise ValidationError("Value '$VALUE' of type '$VALUE_TYPE' could not be converted to Integer")
        return val
...
class BytesNode(ValueNode):
    ...
    def _validate_and_convert_impl(self, value: Any) -> bytes:
        if not isinstance(value, bytes):
            raise ValidationError("Value '$VALUE' of type '$VALUE_TYPE' is not of type 'bytes'")
        return value
...
class FloatNode(ValueNode):
    ...
    def _validate_and_convert_impl(self, value: Any) -> float:
        try:
            ...
            raise ValidationError("Value '$VALUE' of type '$VALUE_TYPE' could not be converted to Float")
...
class BooleanNode(ValueNode):
    ...
    def _validate_and_convert_impl(self, value: Any) -> bool:
        ...
        if isinstance(value, int):
            ...
            raise ValidationError("Value '$VALUE' is not a valid bool (type $VALUE_TYPE)")
...
class EnumNode(ValueNode):  # lgtm [py/missing-equals] : Intentional.
    ...
    def __init__(
        ...
        if not isinstance(enum_type, type) or not issubclass(enum_type, Enum):
            raise ValidationError(f"EnumNode can only operate on Enum subclasses ({enum_type})")
    ...
    def validate_and_convert_to_enum(enum_type: Type[Enum], value: Any) -> Enum:
        if not isinstance(value, (str, int)) and not isinstance(value, enum_type):
            raise ValidationError(f"Value $VALUE ($VALUE_TYPE) is not a valid input for {enum_type}")
        ...
        try:
            ...
            raise ValidationError(f"Invalid value '$VALUE', expected one of [{valid}]").with_traceback(
                sys.exc_info()[2]
            ) from e
