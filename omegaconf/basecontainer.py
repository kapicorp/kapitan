...
class BaseContainer(Container, ABC):
    ...
    @staticmethod
    def _list_merge(dest: Any, src: Any, extend_lists: bool = False, remove_duplicates: bool = False) -> None:
        ...
        if src._is_none():
            ...
            temp_target.__dict__["_metadata"] = copy.deepcopy(dest.__dict__["_metadata"])
            is_optional, et = _resolve_optional(dest._metadata.element_type)
    ...
    def merge_with(
        ...
        *others: Union["BaseContainer", Dict[str, Any], List[Any], Tuple[Any, ...], Any],
        ...
        try:
            self._merge_with(*others, extend_lists=extend_lists, remove_duplicates=remove_duplicates)
    ...
    def _set_item_impl(self, key: Any, value: Any) -> None:
        ...
        if input_is_typed_vnode and not is_union_annotation(target_ref_type):
            ...
            should_set_value = target_node_ref is not None and (
                ...
                or (isinstance(target_node_ref, AnyNode) and is_primitive_type_annotation(value))
            )
            if should_set_value:
                ...
                if (
                    special_value
                    and (is_container_annotation(target_ref_type) or is_structured_config(target_ref_type))
    ...
    def _get_full_key(self, key: Union[DictKeyType, int, slice, None]) -> str:
        ...
        if key is not None and key != "":
            ...
            cur, _ = _select_one(c=self, key=str(key), throw_on_missing=False, throw_on_type_error=False)
            if cur is None:
                cur = self
        ...
        while cur._get_parent() is not None:
            cur = cur._get_parent()
            if id(cur) in memo:
                raise ConfigCycleDetectedException(f"Cycle when iterating over parents of key `{key!s}`")
            memo.add(id(cur))
            assert cur is not None
            if cur._key() is not None:
                full_key = prepand(full_key, type(cur._get_parent()), type(cur), cur._key())
    ...
    if node._is_none():
        ...
        if is_primitive_type_annotation(ref_type) and isinstance(node, ValueNode):
            ...
            if isinstance(node, ValueNode):
                ...
                raise ValidationError(
                    f"'{type(node).__name__}' is incompatible" + f" with type hint '{ref_type}'"
                )
