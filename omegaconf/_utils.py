...
def get_attr_data(obj: Any, allow_objects: Optional[bool] = None) -> Dict[str, Any]:
    ...
    for attrib in get_attr_class_fields(obj):
        ...
        if is_union_annotation(type_) and not is_supported_union_annotation(type_):
            e = ConfigValueError(f"Unions of containers are not supported:\n{name}: {type_str(type_)}")
        ...
        try:
            ...
            format_and_raise(node=dummy_parent, key=name, value=value, cause=ex, msg=str(ex))
...
def get_dataclass_data(obj: Any, allow_objects: Optional[bool] = None) -> Dict[str, Any]:
...
def get_structured_config_data(obj: Any, allow_objects: Optional[bool] = None) -> Dict[str, Any]:
    if is_dataclass(obj):
...
def _is_none(value: Any, resolve: bool = False, throw_on_resolution_failure: bool = True) -> bool:
    from omegaconf import Node
    ...
    if resolve:
        value = value._maybe_dereference_node(throw_on_resolution_failure=throw_on_resolution_failure)
...
def _is_interpolation(v: Any, strict_interpolation_validation: bool = False) -> bool:
    ...
    if isinstance(v, str) and _is_interpolation_string(v, strict_interpolation_validation):
        return True
    return False
...
def format_and_raise(
    ...
    message = s.substitute(REF_TYPE=ref_type_str, OBJECT_TYPE=object_type_str, MSG=msg, FULL_KEY=full_key)
...
def type_str(t: Any, include_module_name: bool = False) -> str:
    ...
    if args is not None:
        args = ", ".join([type_str(t, include_module_name=include_module_name) for t in t.__args__])
        ret = f"{name}[{args}]"
    else:
        ret = name
