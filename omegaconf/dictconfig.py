...
class DictConfig(BaseContainer, MutableMapping[Any, Any]):
    ...
    def __init__(
        ...
        try:
            ...
            if is_structured_config(content) or is_structured_config(ref_type):
                ...
                if is_structured_config_frozen(content) or is_structured_config_frozen(ref_type):
    ...
    def __deepcopy__(self, memo: Dict[int, Any]) -> "DictConfig":
        ...
        res.__dict__["_flags_cache"] = copy.deepcopy(self.__dict__["_flags_cache"], memo=memo)
    ...
    def _is_typed(self) -> bool:
        return self._metadata.object_type not in (Any, None) and not is_dict(self._metadata.object_type)
    ...
    def _validate_set(self, key: Any, value: Any) -> None:
        ...
        target_has_ref_type = isinstance(target, DictConfig) and target._metadata.ref_type not in (Any, dict)
        ...
        if is_dict(value_type) and is_dict(target_type):
            return
        if is_container_annotation(target_type) and not is_container_annotation(value_type):
            raise ValidationError(f"Cannot assign {type_str(value_type)} to {type_str(target_type)}")
        ...
        if _is_none(value, resolve=True, throw_on_resolution_failure=False):
            if key is not None:
                ...
                if child is not None:
                    ...
                    field_is_optional, _ = _resolve_optional(self._metadata.element_type)
            else:
    ...
    def _raise_invalid_value(self, value: Any, value_type: Any, target_type: Any) -> None:
    ...
    def _s_validate_and_normalize_key(self, key_type: Any, key: Any) -> DictKeyType:
        if key_type is Any:
            ...
            if not isinstance(key, key_type):
                raise KeyValidationError(f"Key $KEY ($KEY_TYPE) is incompatible with ({key_type.__name__})")
    ...
    def __setitem__(self, key: DictKeyType, value: Any) -> None:
        try:
            ...
            self._format_and_raise(key=key, value=value, type_override=ConfigKeyError, cause=e)
    ...
    def __getattr__(self, key: str) -> Any:
        ...
        try:
            return self._get_impl(key=key, default_value=_DEFAULT_MARKER_, validate_key=False)
        except ConfigKeyError as e:
            self._format_and_raise(key=key, value=None, cause=e, type_override=ConfigAttributeError)
        except Exception as e:
    ...
    def __getitem__(self, key: DictKeyType) -> Any:
        ...
        try:
            ...
            self._format_and_raise(key=key, value=None, cause=e, type_override=ConfigKeyError)
        except Exception as e:
            self._format_and_raise(key=key, value=None, cause=e)
    ...
    def __delattr__(self, key: str) -> None:
        ...
        if self._get_flag("readonly"):
            self._format_and_raise(
                key=key,
                value=None,
                cause=ReadonlyConfigError("DictConfig in read-only mode does not support deletion"),
            )
        try:
    ...
    def __delitem__(self, key: DictKeyType) -> None:
        ...
        if self._get_flag("struct"):
            self._format_and_raise(
                key=key,
                value=None,
                cause=ConfigTypeError("DictConfig in struct mode does not support deletion"),
            )
    ...
    def _get_impl(self, key: DictKeyType, default_value: Any, validate_key: bool = True) -> Any:
        try:
            node = self._get_child(key=key, throw_on_missing_key=True, validate_key=validate_key)
        ...
        return self._resolve_with_default(key=key, value=node, default_value=default_value)
    ...
    def pop(self, key: DictKeyType, default: Any = _DEFAULT_MARKER_) -> Any:
        try:
            ...
            if node is not None:
                assert isinstance(node, Node)
                value = self._resolve_with_default(key=key, value=node, default_value=default)
                ...
                if default is not _DEFAULT_MARKER_:
                    ...
                    if full != key:
                        raise ConfigKeyError(f"Key not found: '{key!s}' (path: '{full}')")
                    else:
    ...
    def _set_value_impl(self, value: Any, flags: Optional[Dict[str, bool]] = None) -> None:
