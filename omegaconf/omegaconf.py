...
class OmegaConf:
    ...
    def load(file_: Union[str, pathlib.Path, IO[Any]]) -> Union[DictConfig, ListConfig]:
        ...
        if obj is not None and not isinstance(obj, (list, dict, str)):
            raise IOError(f"Invalid loaded object type: {type(obj).__name__}")  # pragma: no cover
    ...
    @staticmethod
    def save(config: Any, f: Union[str, pathlib.Path, IO[Any]], resolve: bool = False) -> None:
        """
    ...
    def unsafe_merge(
        *configs: Union[
        ...
        with flag_override(target, ["readonly", "no_deepcopy_set_nodes"], [False, True]):
            target.merge_with(
                *configs[1:],
                extend_lists=extend_lists,
                remove_duplicates=remove_duplicates,
    ...
    def legacy_register_resolver(name: str, resolver: Resolver) -> None:
        ...
        assert name not in BaseContainer._resolvers, f"resolver '{name}' is already registered"
        ...
        *,
        ...
        def _should_pass(special: str) -> bool:
            ...
            if ret and use_cache:
                raise ValueError(f"use_cache=True is incompatible with functions that receive the {special}")
            return ret
    ...
    def to_container(
        ...
        *,
        ...
        if not OmegaConf.is_config(cfg):
            raise ValueError(f"Input cfg is not an OmegaConf config object ({type_str(type(cfg))})")
    ...
    def update(
        ...
        *,
        ...
        assert isinstance(root, Container), f"Unexpected type for root: {type(root).__name__}"
    ...
    def resolve(cfg: Container) -> None:
        ...
        if not OmegaConf.is_config(cfg):
            ...
            raise ValueError(f"Invalid config type ({type(cfg).__name__}), expected an OmegaConf Container")
            ...
            if isinstance(obj, str):
                ...
                if (
                    ...
                    if isinstance(obj, type):
                        ...
                        raise ValidationError(f"Object of unsupported type: '{type(obj).__name__}'")
    ...
    def _get_resolver(
        name: str,
    ) -> Optional[Callable[[Container, Container, Node, Tuple[Any, ...], Tuple[str, ...]], Any,]]:
        # noinspection PyProtectedMember
        return BaseContainer._resolvers[name] if name in BaseContainer._resolvers else None
...
def _select_one(
    ...
    if isinstance(c, DictConfig):
        ...
        if not is_int(ret_key):
            if throw_on_type_error:
                raise TypeError(f"Index '{ret_key}' ({type(ret_key).__name__}) is not an int")
            else:
                val = None
        else:
            ret_key = int(ret_key)
    ...
    if val is not None:
        ...
        if val._is_missing():
            if throw_on_missing:
                raise MissingMandatoryValue(f"Missing mandatory value: {c._get_full_key(ret_key)}")
            else:
                return val, ret_key
