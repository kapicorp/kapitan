...
class ListConfig(BaseContainer, MutableSequence[Any]):
    ...
    def _validate_get(self, key: Any, value: Any = None) -> None:
        if not isinstance(key, (int, slice)):
            raise KeyValidationError("ListConfig indices must be integers or slices, not $KEY_TYPE")
        ...
        if 0 <= key < self.__len__():
            ...
            if target is not None:
                ...
                if value is None and not target._is_optional():
                    raise ValidationError("$FULL_KEY is not optional and cannot be assigned None")
    ...
    def __getitem__(self, index: Union[int, slice]) -> Any:
        try:
            ...
            if self._is_none():
                raise TypeError("ListConfig object representing None is not subscriptable")

            assert isinstance(self.__dict__["_content"], list)
            if isinstance(index, slice):
                result = []
                start, stop, step = self._correct_index_params(index)
                for slice_idx in itertools.islice(range(0, len(self)), start, stop, step):
                ...
                return self._resolve_with_default(key=index, value=self.__dict__["_content"][index])
    ...
    def insert(self, index: int, item: Any) -> None:
        ...
        try:
            ...
            if self._is_none():
                raise TypeError("Cannot insert into ListConfig object representing None")
            if self._is_missing():
    ...
    def __delitem__(self, key: Union[int, slice]) -> None:
        if self._get_flag("readonly"):
            self._format_and_raise(
                key=key,
                value=None,
                cause=ReadonlyConfigError("Cannot delete item from read-only ListConfig"),
            )
    ...
    def index(self, x: Any, start: Optional[int] = None, end: Optional[int] = None) -> int:
        if start is None:
            start = 0
        if end is None:
            end = len(self)
    ...
    def _get_node(
        ...
        try:
            if self._is_none():
                raise TypeError("Cannot get_node from a ListConfig object representing None")
            if self._is_missing():
    ...
    def pop(self, index: int = -1) -> Any:
        try:
            ...
            self._format_and_raise(key=index, value=None, cause=e, type_override=ConfigTypeError)
            assert False
        except Exception as e:
            self._format_and_raise(key=index, value=None, cause=e)
            assert False
    ...
    def __contains__(self, item: Any) -> bool:
        if self._is_none():
            raise TypeError("Cannot check if an item is in a ListConfig object representing None")
        if self._is_missing():
            raise MissingMandatoryValue("Cannot check if an item is in missing ListConfig")
    ...
    def _set_value_impl(self, value: Any, flags: Optional[Dict[str, bool]] = None) -> None:
        ...
        if _is_none(value):
            if not self._is_optional():
                raise ValidationError("Non optional ListConfig cannot be constructed from None")
