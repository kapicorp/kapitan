...
@pytest.fixture
def isolated_helm_project(tmp_path, monkeypatch):
    """
    ...
    return prepare_isolated_project(
        tmp_path, monkeypatch, KAPITAN_HELM_INTEGRATION, "helm_project"
    )


@pytest.fixture
def isolated_terraform_inventory(tmp_path, monkeypatch):
    """
    Create an isolated copy of the terraform example for test execution.
    Returns the path to the isolated copy.
    """
    isolated_path = prepare_isolated_project(
        tmp_path, monkeypatch, EXAMPLE_TERRAFORM_ROOT, "terraform"
    )
...
@pytest.fixture
def isolated_docker_inventory(tmp_path, monkeypatch):
    """
    Create an isolated copy of the docker example for test execution.
    Returns the path to the isolated copy.
    """
    isolated_path = prepare_isolated_project(
        tmp_path, monkeypatch, EXAMPLE_DOCKER_ROOT, "docker"
    )
