...
KEY = cached.gpg_obj.gen_key(
    cached.gpg_obj.gen_key_input(
        key_type="RSA", key_length=2048, passphrase="testphrase"
    )
)
...
class GPGSecretsTest(unittest.TestCase):
    ...
    def test_gpg_write_reveal(self):
        ...
        REF_CONTROLLER[tag] = GPGSecret(
            "super secret value", [{"fingerprint": KEY.fingerprint}]
        )
    ...
    def test_gpg_write_embedded_reveal(self):
        ...
        file_with_secret_tags = tempfile.mktemp()
        with open(file_with_secret_tags, "w") as fp:
            fp.write(f"I am a file with a {ref_obj.compile()}")
    ...
    def test_gpg_function_ed25519(self):
        ...
        tag_ed25519public = (
            "?{gpg:secret/ed25519public||reveal:secret/ed25519|publickey}"
        )
    ...
    def test_gpg_update_recipients(self):
        ...
        self.assertTrue(
            os.path.isfile(os.path.join(REFS_HOME, "secret/sauce_with_fingerprints"))
        )
